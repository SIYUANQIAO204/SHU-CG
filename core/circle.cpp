...
void RenderEngine::draw_circle_midpoint(const Point &center, int radius) {
    ...
    // 绘制圆的八个对称点
    auto draw_circle_points = [&](int cx, int cy,int index) {
        draw_point(cx + x, cy + y,index);
        draw_point(cx - x, cy + y,index);
        draw_point(cx + x, cy - y,index);
        draw_point(cx - x, cy - y,index);
        draw_point(cx + y, cy + x,index);
        draw_point(cx - y, cy + x,index);
        draw_point(cx + y, cy - x,index);
        draw_point(cx - y, cy - x,index);
    };

    draw_circle_points(center.x, center.y,index);  // 绘制初始点
    ...
    while (x < y) {
        ...
        x++;                                     // x 增加
        draw_circle_points(center.x, center.y,index++);  // 绘制对称点
    }
}
...
void RenderEngine::draw_arc_midpoint(
    ...
    auto draw_arc_points = [&](int x, int y, int index) {
        // 判断点是否在圆弧内
    ...
    // 绘制圆的八个对称点
    auto draw_circle_points = [&](int cx, int cy,int index) {
        draw_arc_points(cx + x, cy + y, index);
        draw_arc_points(cx - x, cy + y, index);
        draw_arc_points(cx + x, cy - y, index);
        draw_arc_points(cx - x, cy - y, index);
        draw_arc_points(cx + y, cy + x, index);
        draw_arc_points(cx - y, cy + x, index);
        draw_arc_points(cx + y, cy - x, index);
        draw_arc_points(cx - y, cy - x, index);
    };
...
// 根据三点求圆心和半径
std::pair<Point, int> RenderCore::circle_center_radius(const Point &p1, const Point &p2, const Point &p3) {
    // 两条中垂线的交点即为圆心
    // 两条中垂线分别为：
